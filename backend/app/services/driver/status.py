    ...
    if elapsed and energy_so_far > 0:
        slot_map = {
            ...
            for row in db.execute(
                select(EnergySlot).where(EnergySlot.timestamp.in_([e.timestamp for e in elapsed]))
            ).scalars().all()
        }
        weighted_carbon = sum(
            e.grid_energy_kwh * slot_map[e.timestamp].carbon_intensity
            for e in elapsed
            if e.timestamp in slot_map
        )
        avg_carbon = weighted_carbon / energy_so_far
        green_score = calculate_green_score(
            renewable_share_pct=renewable_share_pct,
            average_carbon_intensity=avg_carbon,
            grid_energy_kwh=sum(e.grid_energy_kwh for e in elapsed),
            total_energy_kwh=energy_so_far,
        )
    elif entries:
        total_energy = sum(e.energy_kwh for e in entries)
        total_renewable = sum(e.renewable_energy_kwh for e in entries)
        total_grid = sum(e.grid_energy_kwh for e in entries)
        slot_map = {
            row.timestamp: row
            for row in db.execute(
                select(EnergySlot).where(EnergySlot.timestamp.in_([e.timestamp for e in entries]))
            ).scalars().all()
        }
        weighted_carbon = sum(
            e.grid_energy_kwh * slot_map[e.timestamp].carbon_intensity
        ...
        avg_carbon = weighted_carbon / total_energy if total_energy else 0.0
        green_score = calculate_green_score(
            renewable_share_pct=(total_renewable / total_energy * 100.0) if total_energy else 0.0,
            average_carbon_intensity=avg_carbon,
