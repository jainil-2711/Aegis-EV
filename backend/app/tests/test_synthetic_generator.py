...
def test_dataset_sizes_match_settings(settings):
    ...
    assert len(dataset.energy_slots) == (
...
def test_ev_core_invariants(settings):
    """Rules.md core EV invariants."""
    dataset = generate_full_dataset(settings)
    charger_ids = {
        c["id"]
        for c in dataset.chargers
    }
...
def test_station_charger_counts_consistent(settings):
    ...
    for station in dataset.stations:
        ...
        assert station["charger_count"] == len(
            assigned
        )
...
def test_flexibility_classification():
    ...
    # Required 2h charge in a 2h window -> no slack.
    assert compute_flexibility(
        arrival,
    ...
    # Required 2h in a 10h window -> slack 0.8.
    assert compute_flexibility(
        arrival,
        arrival + timedelta(hours=10),
        20.0,
        10.0,
    ) == "high"

    # Required 4h in a 6h window -> roughly 0.33 slack.
    assert compute_flexibility(
        arrival,
...
def test_energy_slots_cover_full_horizon(settings):
    ...
    assert (
        timestamps[0].hour == 0
        and timestamps[0].minute == 0
    )

    span = timestamps[-1] - timestamps[0]

    assert span == timedelta(
        hours=(
            settings.horizon_hours
...
def test_renewable_and_price_bounds(settings):
    ...
    for slot in dataset.energy_slots:
        ...
        assert slot["solar_generation_kw"] >= 0
        assert slot["wind_generation_kw"] >= 0
...
def test_solar_wind_renewable_invariant(settings):
    dataset = generate_full_dataset(settings)
...
def test_generated_ev_targets_are_feasible_within_demo_horizon(
    settings,
):
    ...
    horizon_end = (
        dataset.energy_slots[-1]["timestamp"]
        + timedelta(
    ...
    for ev in dataset.evs:
        ...
        start = max(
            ev["arrival_time"],
            dataset.energy_slots[0]["timestamp"],
        )
...
def test_assigned_chargers_can_deliver_each_generated_request(
    settings,
):
...
def test_multiple_deterministic_seeds_produce_valid_independent_data(
    settings,
):
